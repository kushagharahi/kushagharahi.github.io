<template>
  <div class="post">
    ...
    <span v-if="postMetaData.last_updated" class="postDate"
      >Last Updated {{ postMetaData.last_updated }}</span
    >
    <span v-if="!postMetaData.last_updated" class="postDate"
      >{{ postMetaData.date_posted }}
    </span>
    ...
    <a
      ...
      rel="noopener"
      :href="
        'https://github.com/kushagharahi/kushagharahi.github.io/tree/rc/src/content/blog/posts/' +
        $route.params.name +
        '/post.md'
      "
    ...
    <component :is="'script'" type="application/ld+json">
      ...
      "{{ postMetaData.title }}", "datePublished": "{{ iso8601DateCreated }}",
      "image": ["{{ firstImage }}"], "dateModified": "{{
        iso8601DateModified
      }}", "author": [{ "@type": "Person", "name": "Kusha Gharahi", "url":
      "https://kusha.me" }] }
    </component>
  </div>
</template>
...
export default {
  ...
  created() {
    this.compiledMarkdown = require('posts/' +
      this.$route.params.name +
      '/post.md')
    this.postMetaData = postsJson.filter(
    ...
    this.iso8601DateCreated = new Date(
      this.postMetaData.date_posted.replace(/(th |rd |nd )/, ' '),
    ).toISOString()
    this.iso8601DateModified = new Date(
      (this.postMetaData.last_updated
        ? this.postMetaData.last_updated
        : this.postMetaData.date_posted
      ).replace(/(th |rd |nd )/, ' '),
    ).toISOString()
  },
  methods: {
